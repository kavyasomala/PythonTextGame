...
class TextAdventure:
  ...
  def chat_check(self, response):
    if response.lower in self.exit_commands:
      self.exit
    elif response.lower() in self.affirmative_answers:
      return True
    elif response.lower() in self.negative_answers:
      return False
    else:
      self.unknown_response()

  def chat(self):
    answer = input("\nWould you like this adventure's overview? ")
    self.chat_check(answer)
    if answer:
      exit
    elif not answer:
      self.no_count += 1
...
sublime_adventure = TextAdventure()
sublime_adventure.start_adventure()